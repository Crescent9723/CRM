...
public class OrderListScreenController implements javafx.fxml.Initializable {

	@FXML private AnchorPane pane;
	@FXML private Button btnBack;
	@FXML private TableView<Order> orderListTable;
	@FXML private TableColumn<Order, Integer> orderIDColumn;
	@FXML private TableColumn<Order, String> accountColumn;
	...
	private void setCellValueFactory() {
		...
		accountColumn.setCellValueFactory(new PropertyValueFactory<Order, String>("accountID"));
							...
							if(row.getIndex() != -1){
								...
								check.selectedProperty().addListener(new ChangeListener<Boolean>() {
									...
									public void changed(
										...
										try {
											SQLDatabase.getInstance().setDelivered(data.get(row.getIndex()).getOrderID(), newValue ? 1 : 0);
											updateData();
	...
	private void updateData() {
		try {
			ResultSet rs = SQLDatabase.getInstance().getFullOrderList(true);
			data.clear();
			while (rs.next()){
