		...
		btnDeleteAccount.setOnAction((event) -> {
			...
			if (option == JOptionPane.OK_OPTION){
				try {
					...
					} else if (SQLDatabase.getInstance().checkPassword(passText.getText())){
		...
		btnChangePass.setOnAction((event) -> {
			...
			if (option == JOptionPane.OK_OPTION){
				try {
					if (!oldPassText.getText().equals(oldPassCheckText.getText())){
					...
					} else if (SQLDatabase.getInstance().checkPassword(oldPassText.getText())){
						...
						SceneControl.loadScene(pane, "/ui/LoginScreen.fxml");
					} else {
						JOptionPane.showMessageDialog(null, "Password is incorrect");
					}
				} catch (Exception e) {
